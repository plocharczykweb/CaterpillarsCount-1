...
var caterpillarString = "Caterpillars (Lepidoptera larvae)";
...
$(document).ready(function(){
    ...
    //Listener to make the Caterpillar extras options appear
    $("body").on("change",".arthropod",null, function(){
        var orderName = $(this).val();

        if (orderName == "Caterpillars (Lepidoptera larvae)"){
            var extras = $(this).parent().children("div.caterpillarExtras");

            extras.show();

        }else{
            $(this).parent().children("div.caterpillarExtras").hide();
    ...
    $("div.orderList").on("click", "button.addOrderCard", function(e){
        ...
        if (arthropod == "Caterpillars (Lepidoptera larvae)");{
            hairyOrSpiny = $form.find("[name = hairyOrSpiny]").prop("checked");
            leafRoll = $form.find("[name = leafRoll]").prop("checked");
            silkTent = $form.find("[name = silkTent]").prop("checked");
        }
        if(!arthropod){
    ...
    $("#submitSurvey").click(function (e) {
        ...
        var surveyType = $("input[name=surveyType]").val();
        var leafCount = parseInt($("input[name=leafCount]").val());
        ...
        //Check if the rest of the required fields are filled
                ...
                if (surveyType == '%'){
                    $("p.error").remove();
                    $("#submitSurvey").before("<p class = 'error'>Please select a Survey Type</p>");
                    return;
                }
                if (leafCount == NaN || leafCount < 0){
                    $("p.error").remove();
                    $("#submitSurvey").before("<p class = 'error'>Please enter a valid Leaf Count</p>");
                    return;
                }
...
<option>NONE</option></select><br>");
    ...
    $caterpillarDiv = $("<div class = 'caterpillarExtras' hidden></div>");
    $caterpillarDiv.append("<pre>Hairy or Spiny?: </pre><input type='checkbox' name='hairyOrSpiny' value='hairyOrSpiny'><br>");
    $caterpillarDiv.append("<pre>Leaf Roll?: </pre><input type='checkbox' name='leafRoll' value='leafRoll'><br>");
    $caterpillarDiv.append("<pre>Silk Tent?: </pre><input type='checkbox' name='silkTent' value='silkTent'>");
    $form.append($caterpillarDiv);
...
function createOrderCard($div, i){
    ...
    if (order.arthropod == caterpillarString){
        $div.append("<br><pre>Hairy or Spiny: " + order['hairyOrSpiny'] + "</pre><br>");
        $div.append("<pre>Leaf Roll: " + order['leafRoll'] + "</pre><br>");
        $div.append("<pre>Silk Tent: " + order['silkTent'] + "</pre><br>");
    }
}
...
function submitOrders(surveyID){
    for(var i = 0; i < orders.length; i++){
        ...
        var inputData = {
            ...
            hairyOrSpiny: (orders[i].hairyOrSpiny ? '1' : '0'),
            leafRoll: (orders[i].leafRoll ? '1' : '0'),
            silkTent: (orders[i].silkTent ? '1' : '0')
        }
...
function resetForm(){
    ...
    $("select[name=surveyType]").prop("selectedIndex", '%');
    $("select[name=leafCount]").prop("selectedIndex", '');
