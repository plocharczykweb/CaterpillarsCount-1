<?php
  ...
  for($i = 0; $i < count($lines); $i++){
    ...
    if($densityOrOccurraece == "occurrence"){//occurrence
      ...
      $query = mysqli_query($dbconn, "SELECT Survey.LocalDate, COUNT(*) AS DailyArthropodSightings FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Plant.SiteFK='$siteID' AND ArthropodSighting.Group LIKE '$arthropod' AND YEAR(Survey.LocalDate)='$year' GROUP BY Survey.LocalDate ORDER BY Survey.LocalDate");
      while($row = mysqli_fetch_assoc($query)){
        $dateParts = explode("-", $row["LocalDate"]);
        $month = $dateParts[1];
        $day = $dateParts[2];
